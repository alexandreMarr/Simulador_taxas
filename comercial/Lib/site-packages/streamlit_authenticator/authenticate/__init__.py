...
class Authenticate:
    ...
    def login(self, location: str='main', max_concurrent_users: Optional[int]=None,
        ...
        ----------
        location: str
            Location of the login widget (main or sidebar).
        ...
        -------
        tuple
            (Name of the authenticated user, Status of authentication, Username of the authenticated user).
        """
        
        if fields is None:
            fields = {'form name': 'form name', 'Username': 'Username', 'Password': 'Password',
                    'Login': 'Login', 'nivel': 'nivel'}
        ...
        if not st.session_state.get('authentication_status'):
            ...
            if not st.session_state.get('authentication_status'):
                ...
                # Adicione o logotipo acima dos campos de entrada
                login_form.image("https://i.ibb.co/R4xBj4V/Rovema-Pay.jpg", width=200)
                ...
                if login_form.form_submit_button('Login' if 'Login' not in fields else fields['Login']):
                    if self.authentication_handler.check_credentials(username, password, max_concurrent_users, max_login_attempts):
                        self.authentication_handler.execute_login(username=username)
                        self.cookie_handler.set_cookie()
        
        return (st.session_state.get('name'), st.session_state.get('authentication_status'),
                st.session_state.get('username'), st.session_state.get('nivel'))
    ...
    def register_user(self, location: str='main', pre_authorization: bool=True,
        ...
        new_password_repeat = register_user_form.text_input('Repeat password'
                                                            ...
                                                            type='password')
        nivel = register_user_form.selectbox('Nivel de Acesso'
                                                            if 'Nivel' not in fields
                                                            else fields['Nivel'],
                                                            ['admin','admin_master','user'])
        if register_user_form.form_submit_button('Register' if 'Register' not in fields
            ...
            return self.authentication_handler.register_user(new_password, new_password_repeat,
                                                             ...
                                                             new_name, new_email, nivel, domains)
    ...
    def update_user_details(self, username: str, location: str='main', fields: dict=None,
        ...
        """
        if fields is None:
            fields = {'Form name':'Update'} 
        if location not in ['main', 'sidebar']:
            # Temporary deprecation error to be displayed until a future release
            raise DeprecationError("""Likely deprecation error, the 'form_name' parameter
                                   has been replaced with the 'fields' parameter. For further
                                   information please refer to user details', 'Field':'Field', 'Name':'Name',
                      'Email':'Email', 'New value':'New value', 'Update':
